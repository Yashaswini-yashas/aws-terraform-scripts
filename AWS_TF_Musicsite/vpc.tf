...
/*
  Jenkins Instance configuration - Windows
*/
...
resource "aws_network_interface" "jenkins_Windows" {
  subnet_id = "${aws_subnet.us-west-2-private.id}"
  private_ips = ["10.0.2.20"]
  security_groups = ["${aws_security_group.jenkins_Windows.id}"]
  tags = {
    Name = "Jenkins_Windows_network_interface"
  }
  depends_on = [aws_instance.nat]
}
...
resource "aws_instance" "jenkins_Windows" {
    ...
    key_name = "Prathamesh_Oregon"
	user_data = "${file("jenkinsWindows.txt")}"
	network_interface {
    network_interface_id = "${aws_network_interface.jenkins_Windows.id}"
    device_index         = 0
	}
    tags = {
        Name = "Lab_Jenkins_Windows_Instance"
    }
	depends_on = [aws_instance.nat]
}
